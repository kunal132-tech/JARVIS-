import asyncio
import threading
import os
import edge_tts
import pygame
from playsound import playsound


VOICE = "en-AU-WilliamNeural"
BUFFER_SIZE = 1024


def remove_file(file_path):
    max_attempts = 3
    attempts = 0
    while attempts < max_attempts:
        try:
            with open(file_path, 'w'):
                pass  # Open and close the file to create an empty file
            os.remove(file_path)
            break
        except Exception as e:
            print(f"Error: {e}")
            attempts += 1
        finally:
            pass  # Make sure the file is closed


async def amain(TEXT, output_file) -> None:
    try:
        communicate_txt = edge_tts.Communicate(TEXT, VOICE)
        await communicate_txt.save(output_file)
        thread = threading.Thread(target=play_audio, args=(output_file,))
        thread.start()
        thread.join()
    except Exception as e:
        print(f"Error : {e}")
    finally:
        remove_file(output_file)


def play_audio(file_path):
    try:
        pygame.init()
        pygame.mixer.init()

        if not os.path.exists(file_path):
            raise FileNotFoundError(f"File not found: {file_path}")

        sound = pygame.mixer.Sound(file_path)
        sound.play()

        while pygame.mixer.get_busy():
            pygame.time.Clock().tick(10)

        pygame.quit()

    except Exception as e:
        print(f"Error during audio playback: {e}")


def speak(TEXT, output_file=None):
    if output_file is None:
        output_file = os.path.join(os.getcwd(), "speak.mp3")
    asyncio.run(amain(TEXT, output_file))

